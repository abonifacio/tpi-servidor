...
function Storage(){
    ...
    this.add = (disp)=>{
        ...
        if(ports.exists(disp.mac)){
            ...
            ex.sampleRate = disp.sampleRate || 8000
            ex.sampleSize = disp.sampleSize || 8
            ex.stereo = disp.stereo
            log.debug('Dispositivo existente: ',ex)
            if(conf.db){
                return db.update(ex).then((data)=>new Dispositivo(data,ex.puerto))
            }
        ...
        if(conf.wav){
            ...
            promises.push(wavFiles.init(nw))
        }
        if(!promises.length) return Promise.resolve(nw)
        return Promise.all(promises).then(([disp,file])=>{
            return new Dispositivo(disp || nw,nw.puerto,file && [file])
        })
    }
    ...
    this.buffer = (bytes,dispositivo)=>{
        if(conf.wav){
            return wavFiles.buffer(bytes,dispositivo)
        }
    }
